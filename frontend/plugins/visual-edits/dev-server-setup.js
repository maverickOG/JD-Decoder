      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="dev@jddecoder.local" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="dev@jddecoder.local" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
